  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <Card className="p-6 bg-card border border-border">
        <h4 className="text-sm font-medium text-muted-foreground mb-3">Sharpe Ratio</h4>
        <p className="text-3xl font-bold text-cyan-400">{metrics.sharpeRatio.toFixed(2)}</p>
        <p className="text-xs text-muted-foreground mt-2">Risk-adjusted returns</p>
      </Card>

      <Card className="p-6 bg-card border border-border">
        <h4 className="text-sm font-medium text-muted-foreground mb-3">Max Drawdown</h4>
        <p className="text-3xl font-bold text-red-400">${metrics.maxDrawdown.toFixed(2)}</p>
        <p className="text-xs text-muted-foreground mt-2">Peak to trough</p>
      </Card>

      <Card className="p-6 bg-card border border-border">
        <h4 className="text-sm font-medium text-muted-foreground mb-3">Profit Factor</h4>
        <p className="text-3xl font-bold text-emerald-400">{metrics.profitFactor.toFixed(2)}</p>
        <p className="text-xs text-muted-foreground mt-2">Gross profit / Gross loss</p>
      </Card>

      <Card className="p-6 bg-card border border-border">
        <h4 className="text-sm font-medium text-muted-foreground mb-3">Avg Trade</h4>
        <p className={`text-3xl font-bold ${metrics.avgTrade >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
          ${metrics.avgTrade.toFixed(2)}
        </p>
        <p className="text-xs text-muted-foreground mt-2">Average per trade</p>
      </Card>
    </div>
  );
}
